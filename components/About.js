import Image from "next/dist/client/image";

const About = () => {
  return (
    <div id="about" className="about-container bg-white h-screen py-16">
      <div className="hero grid grid-cols-1  md:grid-cols-8 border-b-2 border-green-600 bg-gray-100 mx-4 md:mx-16 p-4 md:p-16">
        <h1 className="order-last md:order-first md:col-span-6 text-lg md:text-4xl md:leading-relaxed">
          Hello, I'm <span className="span line">Mick</span>,<br /> an
          experienced <span className="span2">graphic designer</span> and{" "}
          <span className="span2">dtp-operator.</span> <br />
          Currently on a <span className="span line">journey</span> of becoming
          a <span className="span">web developer</span>.
        </h1>
        <div className="portrait col-span-2">
          <Image
            layout="responsive"
            objectFit="cover"
            src={"/portret/portret_mick_duotoon_2021.jpg"}
            alt={"mick"}
            width={200}
            height={200}
            className={"image"}
          />
        </div>
      </div>
    </div>
  );
};

export default About;
